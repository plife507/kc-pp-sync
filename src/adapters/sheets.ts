...
function isMarginHeader(value: unknown): boolean {
  const raw = String(value ?? "").trim();
  return normalizeHeaderCell(raw) === "margin" || /^\d+(?:\.\d+)?%$/.test(raw);
}

function headerMatches(row: string[], index: number, aliases: string[]): boolean {
...
export function validateSourceTabHeader(tabName: string, headerRow: string[]): SourceTabLayoutValidation {
  const layout: SourceTabLayout = tabName.endsWith(" - R")
    ? "recurring"
    : isNewLayout(tabName)
      ? "new-one-off"
      : "legacy-one-off";
...
export async function assertSourceTabLayout(spreadsheetId: string, tabName: string): Promise<void> {
  const sheets = await getSheetsClient();
  ...
  if (!validation.ok) {
    throw new Error(
      `Tab "${tabName}" does not match expected ${validation.layout} layout: ${validation.errors.join("; ")}. Refusing to sync until the tab is repaired.`,
    );
  }
}
...
export function getLogTrimCount(totalUsedRows: number, maxRows: number = LOG_MAX_ROWS): number {
  // Keep the Log sheet capped to maxRows total rows, including the header.
  return Math.max(0, totalUsedRows - maxRows);
}

async function trimSyncLogRows(
...
function recurringInvoiceKey(invoiceNum: string): string {
...
export function groupRecurringRowsByInvoice(
  rows: RecurringProfitabilityRow[],
  getWeekKey: (dateStr: string) => string | null,
): RecurringProfitabilityInvoiceGroup[] {
  const groups = new Map<string, RecurringProfitabilityInvoiceGroup>();
  ...
  for (const row of rows) {
    ...
    if (!group) {
      ...
      if (group.invoiceTotal === 0 && invoiceTotal > 0) group.invoiceTotal = invoiceTotal;
      if (weekKey && (!group.weekKey || weekKey < group.weekKey)) group.weekKey = weekKey;
    }
  ...
  for (const m of MONTHS_TO_SCAN) {
    ...
    if (hasRecurring) {
      try {
        ...
        const rows = (res.data.values ?? []) as string[][];
        const recurringRows: RecurringProfitabilityRow[] = [];
        ...
        for (const group of groupRecurringRowsByInvoice(recurringRows, getWeekSunday)) {
          ...
          if (group.clientPaid && group.labor > 0 && group.invoiceTotal > 0) {
            ...
            recurringExcluded++;
            if (group.weekKey) getOrCreateWeek(group.weekKey).recurringExcluded.add(group.invoiceNum);
          }
        }
      } catch (e: any) {
        if (!e?.message?.includes("Unable to parse range")) throw e;
