...
interface TokenCache {
  accessToken: string;
  obtainedAt: number;   // unix ms when token was obtained
  expiresAt: number;    // unix ms when token expires (obtainedAt + 60min)
}
...
function loadTokenCache(): TokenCache | null {
  try {
    ...
    if (cache.accessToken && cache.expiresAt) return cache;
  } catch {
...
function isCachedTokenValid(cache: TokenCache): boolean {
  return Date.now() + MIN_TOKEN_REMAINING_MS < cache.expiresAt;
}

function canAttemptSignIn(cache: TokenCache | null): boolean {
  if (!cache) return true; // no prior signIn recorded
...
async function gqlFetch<T>(
  ...
  if (res.status === 429 || res.status === 403) {
    throw new Error(
      `HeyPros BLOCKED (HTTP ${res.status}). ` +
  ...
  if (json.errors?.length) {
    ...
    if (messages.toLowerCase().includes("too many") || messages.toLowerCase().includes("try again")) {
      throw new Error(
        `HeyPros AUTH THROTTLED: ${messages}. ` +
...
async function getValidToken(config: Config): Promise<string> {
  ...
  if (!canAttemptSignIn(cache)) {
    ...
    throw new Error(
      `HeyPros: token expired but signIn rate limit active. ` +
  ...
  try {
    const data = await gqlFetch<{
      signIn: { accessToken: string | null; refreshToken: string | null };
    }>(
      config.heypros.graphqlUrl,
      config.heypros.tenant,
      null,
      SIGN_IN_MUTATION,
      { email: config.heypros.email, password: config.heypros.password },
    );
    ...
    if (err instanceof Error) {
      throw new Error(
        `HeyPros signIn FAILED: ${err.message}. ` +
...
interface JobsByPOItem {
  hashid: string;
  hashidNumeric: string;
  purchaseOrder: string | null;
  name: string;
  statusV2: { label: string };
  jobLabelInstances: { label: { hashid: string; name: string } }[];
 ...
 *   "19633"             → ["19633"]
 */
export function parsePurchaseOrder(raw: string | null | undefined): string[] {
  if (!raw) return [];
...
export async function fetchJobsByPurchaseOrders(
  config: Config,
  purchaseOrders: string[],
): Promise<HeyProsJobDetail[]> {
  const token = await getValidToken(config);
  const poSet = new Set(purchaseOrders);
  const matched: HeyProsJobDetail[] = [];
  let page = 1;
  let totalPages = 1;
