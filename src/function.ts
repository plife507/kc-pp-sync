...
function invoiceLookupKey(invoiceNumber: string | null | undefined): string {
  const raw = String(invoiceNumber ?? "").trim();
  const digits = raw.replace(/\D/g, "");
  return digits ? String(parseInt(digits, 10)) : raw.toLowerCase();
}

function findInvoiceRecord(records: JobberPaidJob[], invoiceNumber: string): JobberPaidJob | undefined {
...
function getDatePartsInTimeZone(iso: string, timeZone: string): { month: number; day: number; year: number } | null {
...
function isHeyProsWOInTargetMonth(wo: HeyProsJobDetail, tabMonth: { month: number; year: number } | null): boolean {
  if (!tabMonth || !wo.installationStarts) return true;
...
function formatHeyProsServiceDate(iso: string | null | undefined): string {
  if (!iso) return "";
...
async function runRecurringTabFlow(config: Config): Promise<{ updateCount: number; jobCount: number }> {
  ...
  const invoiceByNumber = new Map<string, JobberPaidJob[]>();
  ...
  for (const { rowIndex, jobNumber, invoiceNumber, heyProsId } of rows) {
    ...
    if (!jobRecord) plainParts.push("⚠️ Job# not found in Jobber");
  ...
  try {
    ...
    if (bodyMode && typeof bodyMode === "string") {
      ...
      if (resolved === "__all_prev__") {
        ...
        const sheetsApi = google.sheets({ version: "v4", auth: gauth as any });
        ...
        const olderMonths: string[] = [];
        for (const tabName of allTabs) {
          const parsed = parseTabMonth(tabName);
          if (!parsed) continue;
          if (tabName.includes(" - R") || tabName.includes(" - GTP")) continue;
    ...
    if (req.body?.fixColVCF) {
      const tabName = req.body.fixColVCF as string;
      const { google } = await import("googleapis");
      const gauth = new google.auth.GoogleAuth({ scopes: ["https://www.googleapis.com/auth/spreadsheets"] });
      const sheets = google.sheets({ version: "v4", auth: gauth as any });
      const meta = await sheets.spreadsheets.get({
      ...
      for (let i = 0; i < cf.length; i++) {
        ...
        const isColV = ranges.some((r: any) =>
          r.startColumnIndex === 21 && (r.endColumnIndex === 22 || !r.endColumnIndex)
        );
        if (!isColV) continue;
        ...
        if (cond.type === "TEXT_EQ") {
          ...
          if (statusValues.has(val)) toDelete.push(i);
        } else if (cond.type === "CUSTOM_FORMULA") {
          const formula = cond.values?.[0]?.userEnteredValue || "";
          if (formula.includes("No Payment") || formula.includes("NO CLIENT PAY")) toDelete.push(i);
        }
      }
      ...
      for (let i = 0; i < cf.length; i++) {
        ...
        if (cond.type === "NOT_BLANK") toDelete.push(i); // NOT_BLANK blocks status colors
      }
      ...
      const existingColV = new Set<string>();
      for (const rule of cf) {
        const ranges = rule.ranges || [];
        const isColV = ranges.some((r: any) => r.startColumnIndex === 21 && r.endColumnIndex === 22);
        if (!isColV) continue;
        const cond = rule.booleanRule?.condition;
        if (cond?.type === "TEXT_EQ") existingColV.add(cond.values?.[0]?.userEnteredValue || "");
        if (cond?.type === "NOT_BLANK") existingColV.add("NOT_BLANK");
      }

      const addRequests: any[] = [];

      // Add missing rules to Col T (19)
      const colTRules: Array<{type: string; value?: string; formula?: string; bg: any}> = [
      ...
      for (const r of colTRules) {
        ...
        if (existingColT.has(key)) continue;
        addRequests.push({
          addConditionalFormatRule: {
            rule: {
    ...
    if (req.body?.renameTab) {
      const { from, to } = req.body.renameTab as { from: string; to: string };
    ...
    if (!config.sheets.dryRun) {
      ...
      let dashboardMarginMap: Map<string, number> | undefined;
      try {
