 ...
 */
async function ensureGridColumns(spreadsheetId, tab, minCols) {
    const sheets = await getSheetsClient();
    const meta = await sheets.spreadsheets.get({ spreadsheetId, fields: "sheets.properties" });
    const sheetProps = (meta.data.sheets ?? []).find(s => s.properties?.title === tab);
    if (!sheetProps)
        return; // tab not found, will fail elsewhere
...
export async function refreshGTPTab(spreadsheetId, sourceTab) {
    ...
    function extractGtpRows(rows, layoutNew, recurring = false) {
...
export async function refreshProfitabilityDashboard(spreadsheetId) {
    ...
    for (const monthName of MONTH_NAMES) {
        if (monthName === "January")
            continue; // January excluded per Nathan
        ...
        if (tabSet.has(monthName)) {
            ...
            for (let year = 2025; year <= 2030; year++) {
                if (tabSet.has(`${monthName} ${year}`)) {
                    oneOffTab = `${monthName} ${year}`;
                    break;
                }
            }
        }
                ...
                if (m.type === "legacy") {
                    ...
                    for (const row of rows) {
                        ...
                        if (clientPaid && labor > 0) {
                            ...
                            oneOffExcluded++;
                            if (weekKey)
                                getOrCreateWeek(weekKey).oneOffExcluded++;
                        }
                    }
                }
                else {
                    ...
                    for (const row of rows) {
                        ...
                        if (clientPaid && labor > 0) {
                            ...
                            if (weekKey) {
                                if (!weekSeenJobs.has(weekKey))
                                    weekSeenJobs.set(weekKey, new Set());
                                ...
                                if (!wkSeen.has(jobNum)) {
                                    ...
                                    const wk = getOrCreateWeek(weekKey);
                                    if (isHybrid)
                                        wk.hybridRevenue += rev;
                                    else
                                        wk.oneOffRevenue += rev;
                                }
                            }
                        }
                        else if (!isHybrid) {
    ...
    if (monthData.length === 0 && weekMap.size === 0)
        return new Map();
 ...
 */
export async function extendTabCF(spreadsheetId, tabName, maxRow = 500) {
    const sheets = await getSheetsClient();
    const meta = await sheets.spreadsheets.get({
        spreadsheetId,
    ...
    const toExtend = cf
        ...
        return ranges.some((r) => r.sheetId === sheetId && (r.endRowIndex || 0) < maxRow);
    });
    ...
    const requests = toExtend.map(({ rule, index }) => ({
 ...
 */
export function buildReleasedBelowSubInvoiceFormula(subCol, releaseCol) {
    const numeric = (col) => `IFERROR(VALUE(REGEXREPLACE(TO_TEXT($${col}2),"[^0-9.-]","")),0)`;
...
export async function setupReleasedBelowSubInvoiceCF(spreadsheetId, tabName) {
    const sheets = await getSheetsClient();
    const meta = await sheets.spreadsheets.get({
        spreadsheetId,
        fields: "sheets(properties,conditionalFormats)",
    ...
    for (let i = cf.length - 1; i >= 0; i--) {
        ...
        if (cond?.type !== "CUSTOM_FORMULA")
            continue;
        const formula = cond.values?.[0]?.userEnteredValue || "";
        const targetsAmountRange = (cf[i]?.ranges || []).some((r) => r.sheetId === sheetId &&
            r.startColumnIndex === subIdx &&
            r.endColumnIndex === releaseIdx + 1);
        const comparesAmountPair = formula.includes(`$${subCol}2`) &&
...
async function deleteExistingEmptyTab(sheets, spreadsheetId, tabName, safetyRanges) {
    const meta = await sheets.spreadsheets.get({
        spreadsheetId,
        fields: "sheets.properties(sheetId,title)",
    });
...
async function duplicateTemplateTab(sheets, spreadsheetId, templateTab, newTab) {
    const meta = await sheets.spreadsheets.get({
        spreadsheetId,
    ...
    if (!source)
        throw new Error(`Template tab "${templateTab}" not found`);
    ...
    if (newSheetId == null)
        throw new Error(`Duplicate of "${templateTab}" did not return a sheetId`);
    await sheets.spreadsheets.batchUpdate({
        spreadsheetId,
        requestBody: {
            requests: [{
...
export async function setupMonthTabs(spreadsheetId, monthName, options = {}) {
    ...
    const result = { month: monthName, created: [], replaced: [], skipped: [] };
    const targets = [
    ...
    for (const target of targets) {
        ...
        if (exists) {
            ...
            if (replaced)
                result.replaced.push(target.tab);
        }
